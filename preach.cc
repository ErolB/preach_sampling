...
// this recursively finds all paths from the first cut to the current node
vector<int> step_back(ListDigraph& g, Cut cut1, Cut cut2, ListDigraph::Node start_node){
    vector<int> edges;
    vector<int> previous_edges;
    for(ListDigraph::InArcIt arc(g, start_node); arc != INVALID; ++arc) {
        ListDigraph::Node origin = g.source(arc);
        bool valid = true;
        FOREACH_BS(nodeID, cut1.getMiddle()) {
            if (g.id(origin) == nodeID) {  // if the source node is on the previous cut
                valid = false;
                cout << "stopped" << endl;
            }
        }
        edges.push_back(g.id(arc));
        if (valid) {
            previous_edges = step_back(g, cut1, cut2, origin);
            for (int i = 0; i < previous_edges.size(); i++){
                edges.push_back(previous_edges[i]);
            }
        }
    }
    return edges;
...
// Examines two cuts and finds the horizontal cuts between them
void FindHorizontalCuts(ListDigraph& g, Cut cut1, Cut cut2, vector<vector<int>>& horizontal_cuts){
    vector<vector<int>> paths;
    FOREACH_BS(nodeID, cut2.getMiddle()){
        vector<int> nodes;
        nodes = step_back(g, cut1, cut2, g.nodeFromId(nodeID));
        if (nodes.size()) {
...
void ConsumeSausage(ListDigraph& g, WeightMap& wMap, Polynomial& poly, Edges_T& sausage, Nodes_T& endNodes){
    ...
    // if edge shuffling is enabled, the list will be randomly rearranged
    if (SHUFFLE){
        vector<int> temp = edges; // temporarily holds the contents of the edge vector
        edges.clear();
        int edge_count = temp.size();
        for (int i = 0; i < edge_count; i++){
            int selection = rand() % temp.size();
            edges.push_back(temp[selection]);
    ...
    */
    ...
    for (int i = 0; i < edges.size(); i++){
        ...
        // record current polynomial size
        lengths.push_back(poly.getSize());
    }

    // find largest size during calculation
    int maximum = 0;
    ...
    cout << "Maximum polynomial length: " << maximum << endl;
...
double Solve(ListDigraph& g, WeightMap& wMap, ListDigraph::Node& source, ListDigraph::Node& target, vector<Cut>& cuts){
    ...
    while(cuts.size() > 0){
        ...
        // print results
        cout << "Cut #" << cut_counter << ": " << (end - start) << endl;
        //cout << "Covered edges: ";
        //FOREACH_BS(id, sausage){
        //    cout << id << ", ";
        //}
        cout << endl;
        cut_counter++;
...
double iteration(ListDigraph& gOrig, WeightMap& wMapOrig, ArcIntMap& arcIdMapOrig,
    ...
    FindHorizontalCuts(g, cuts[1], cuts[2], temp);
