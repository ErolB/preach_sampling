...
dataset, output, sample_prob, sample_size, method, probe_start, probe_end, probe_step = ARGV
...
open("#{output}/times_rand_#{sample_prob}_#{sample_size}.out", "w"){|f.puts "#{probe_start}\t\t#{time}\n#{probe_end}\t\t#{time}"|}