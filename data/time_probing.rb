...
probe_start.step probe_end, probe_step do |probe_size|
	...
	run_repeats.times do
		puts "../preach #{dataset}/network.txt #{dataset}/sources.txt #{dataset}/targets.txt #{sample_prob} #{sample_size} #{method} #{probe_size} #{probe_repeats}"
		time = `(time -p ../preach #{dataset}/network.txt #{dataset}/sources.txt #{dataset}/targets.txt #{sample_prob} #{sample_size} #{method} #{probe_size} #{probe_repeats}) 2>&1 | grep real | tr -d '\n' | cut -d" " -f2`.strip.to_f
...
time = 0.0
run_repeats.times do
	puts "../preach #{dataset}/network.txt #{dataset}/sources.txt #{dataset}/targets.txt #{sample_prob} #{sample_size} rand 10 10"
	time = `(time -p ../preach #{dataset}/network.txt #{dataset}/sources.txt #{dataset}/targets.txt #{sample_prob} #{sample_size} rand 10 10) 2>&1 | grep real | tr -d '\n' | cut -d" " -f2`.strip.to_f
